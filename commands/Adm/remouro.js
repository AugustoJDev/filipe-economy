...
module.exports = {
    ...
    async execute(client, message, args, database) {
        ...
        if (!usuario) return message.reply({ embeds: [errorEmbed.setDescription('Você não mencionou ninguém. `b.remouro <@usuario> <valor>`')] })
        if (!valor) return message.reply({ embeds: [errorEmbed.setDescription('Você não inseriu o valor. `b.remouro <@usuario> <valor>`')] })
        ...
        message.channel.send({ embeds: [sucEmbed.setDescription(`\`${usuario.user.tag}\` perdeu **${new Intl.NumberFormat('de-DE').format(valor)}** ouros.`)] })
    }
}